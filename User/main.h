...
void TIM4_InterruptMode(void);

void Wheels_Control(int RF_Tag,int LF_Tag,int LB_Tag,int RB_Tag);
void Remote_Signals_Receive(void);
int Set_CHECK(int SET);
void WheelCon(int ID,int SpeSET);
void Encoder_CHECK(struct PID_WheelTypeDef *PID);

float Pitch_Increment(uint16_t Data);
float Yaw_Increment(uint16_t Data);

extern struct PID_WheelTypeDef PID_Wheel_201;
extern struct PID_WheelTypeDef PID_Wheel_202;
extern struct PID_WheelTypeDef PID_Wheel_203;
extern struct PID_WheelTypeDef PID_Wheel_204;
