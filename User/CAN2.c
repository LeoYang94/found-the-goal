
#include "CAN2.h"
#include "Holder.h"
#include "Chassis_Control.h"
#include "Wheel_Speed.h"

CanRxMsg RxMessage205;
CanRxMsg RxMessage206;
CanRxMsg RxMessage;
int16_t car_yaw;

/***********************************************************************************
  * @函数描述：  CAN1通信初始化，波特率1MHz. CAN1_TX----PA12,CAN1_RX-----PA11
  * @入口参数：  无.
  * @返回值  :   无.
***********************************************************************************/
void CAN2_Configuration(void)
{
   CAN_InitTypeDef        can;
    CAN_FilterInitTypeDef  can_filter;
    GPIO_InitTypeDef       gpio;
    NVIC_InitTypeDef       nvic;

    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB, ENABLE);
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_CAN1, ENABLE);
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_CAN2, ENABLE);//CAN2用之前需要CAN1enable吗

    GPIO_PinAFConfig(GPIOB, GPIO_PinSource13, GPIO_AF_CAN2);
    GPIO_PinAFConfig(GPIOB, GPIO_PinSource12, GPIO_AF_CAN2); 

    gpio.GPIO_Pin = GPIO_Pin_13 | GPIO_Pin_12 ;
    gpio.GPIO_Mode = GPIO_Mode_AF;//配置端口为复用模式
    GPIO_Init(GPIOB, &gpio);

    nvic.NVIC_IRQChannel = CAN2_RX0_IRQn;//接收中断配置
    nvic.NVIC_IRQChannelPreemptionPriority = 0;
    nvic.NVIC_IRQChannelSubPriority = 1;
    nvic.NVIC_IRQChannelCmd = ENABLE;
    NVIC_Init(&nvic);

//		nvic.NVIC_IRQChannel = CAN2_TX_IRQn;
//    nvic.NVIC_IRQChannelPreemptionPriority = 0;
//    nvic.NVIC_IRQChannelSubPriority = 5;
//    nvic.NVIC_IRQChannelCmd = ENABLE;
//    NVIC_Init(&nvic);
//		
		
    CAN_DeInit(CAN2);
    CAN_StructInit(&can);

    can.CAN_TTCM = DISABLE;
    can.CAN_ABOM = DISABLE;    
    can.CAN_AWUM = DISABLE;    
    can.CAN_NART = DISABLE;    
    can.CAN_RFLM = DISABLE;    
    can.CAN_TXFP = DISABLE;     
    can.CAN_Mode = CAN_Mode_Normal; //模式设置
    can.CAN_SJW  = CAN_SJW_1tq;//重新同步跳跃宽度为一个时间单位
    can.CAN_BS1 = CAN_BS1_9tq;//时间段1占用9个时间单位
    can.CAN_BS2 = CAN_BS2_4tq;//时间段2占用4个时间单位
    can.CAN_Prescaler = 3;   //CAN BaudRate 42/(1+9+4)/3=1Mbps
    CAN_Init(CAN2, &can);
    //设置滤波器
    can_filter.CAN_FilterNumber=14;
    can_filter.CAN_FilterMode=CAN_FilterMode_IdMask;
    can_filter.CAN_FilterScale=CAN_FilterScale_32bit;
    can_filter.CAN_FilterIdHigh=0x0000;
    can_filter.CAN_FilterIdLow=0x0000;
    can_filter.CAN_FilterMaskIdHigh=0x0000;
    can_filter.CAN_FilterMaskIdLow=0x0000;
    can_filter.CAN_FilterFIFOAssignment=0;//the message which pass the filter save in fifo0
    can_filter.CAN_FilterActivation=ENABLE;
    CAN_FilterInit(&can_filter);
    
    CAN_ITConfig(CAN2,CAN_IT_FMP0,ENABLE);
    //CAN_ITConfig(CAN2,CAN_IT_TME,ENABLE);

}


void CAN2_TX_IRQHandler(void) //CAN TX
{
  if (CAN_GetITStatus(CAN2,CAN_IT_TME)!= RESET)    //if transmit mailbox is empty ,Transmit mailbox empty Interrupt
  {
	   CAN_ClearITPendingBit(CAN2,CAN_IT_TME);   
  }
}


/***************************************************************************************
  * @函数描述：  CAN1接收中断函数,依据ID将数据送入不同全局变量
  * @入口参数：  无.
  * @返回值  :   无.
****************************************************************************************/
void CAN2_RX0_IRQHandler(void)
{
  
	
  if (CAN_GetITStatus(CAN2,CAN_IT_FMP0)!= RESET)//FIFO 0 message pending Interrupt
	{
		CAN_ClearITPendingBit(CAN2, CAN_IT_FMP0);
		CAN_Receive(CAN2,CAN_FIFO0,&RxMessage);
		
		if( RxMessage.StdId == 0x206 )//Pitch编码器值
		{
	
			Pitch_Hold_Info.can_angle_raw = RxMessage.Data[0] * 256 + RxMessage.Data[1];
			
//						if( Pitch_Hold_Info.can_angle_raw > 7000 )
//			{
//				Pitch_Hold_Info.can_angle_raw = Pitch_Hold_Info.can_angle_raw - 8192;
//			}	
//			Pitch_Hold_Info.can_angle=0.04132f *(Pitch_Hold_Info.can_angle_raw -7733);
			RxMessage205 = RxMessage;
		}
		else if(  RxMessage.StdId == 0x205 )//chassis,yaw
		{
			Chassis_Control_Info.Chassis_angle_raw = RxMessage.Data[0] * 256 + RxMessage.Data[1];
			yaw_Hold_Info.can_angle_raw = Chassis_Control_Info.Chassis_angle_raw;
			RxMessage206 = RxMessage;

//			yaw_Hold_Info.can_angle=0.04132f *(yaw_Hold_Info.can_angle_raw-5465);
		}
		
		else if( (RxMessage.StdId >= 0x201)&&( RxMessage.StdId <= 0x204 ) )
		{
			Four_Wheel_Info.position[RxMessage.StdId - 0x201] = RxMessage.Data[0]*256 +RxMessage.Data[1];
			
			Four_Wheel_Info.speed_raw[RxMessage.StdId - 0x201] = RxMessage.Data[2]*256 +RxMessage.Data[3];
		}
		

   }
}


void Motro_test_out(uint16_t id  , float * speed_list )
{
	int16_t i16_speed_list[4];
	int8_t i;
	uint8_t TransmitMailbox;
	
	int32_t t;
	
	CanTxMsg TxMessage;
	TxMessage.StdId = id;	
	TxMessage.IDE=CAN_ID_STD;					 //STANDARD MODE
  TxMessage.RTR=CAN_RTR_DATA;				 //发送的是数据
  TxMessage.DLC=8;							 //数据长度为2字节
	

	

	for( i=0;i<4;i++ )
	{
		i16_speed_list[i] = (int16_t)(*(speed_list+i));
		
		TxMessage.Data[i*2+0] = (uint8_t)(i16_speed_list[i]>>8);
		TxMessage.Data[i*2+1] = (uint8_t)(i16_speed_list[i]);
	}

	
	TransmitMailbox=CAN_Transmit(CAN2,&TxMessage);
	t=0;
	while((CAN_TransmitStatus(CAN2,TransmitMailbox)!=CANTXOK)&&(t<4000))//CAN_TxStatus_Ok
	{
		t++;
	}

}



